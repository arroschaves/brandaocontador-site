...
def sync_fazenda_folders():
    ...
    for u in unidades:
        ...
        # Verifica se a pasta da fazenda já existe na raiz do cliente
        ...
        if not res.get('files'):
            log(f"📁 Criando pasta da Fazenda '{fazenda_name}' para o cliente '{client['nome']}'")
            file_metadata = {
                'name': fazenda_name,
                'parents': [parent_id],
            ...
            log(f"✅ Fazenda '{fazenda_name}' já possui pasta para '{client['nome']}'")
