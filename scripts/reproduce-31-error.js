...
async function reproduceError() {
    ...
    console.log('--- REPRODUZINDO ERRO COM PAYLOAD COMPLETO ---');
    const { data, error } = await supabase.from('clientes').insert([fullPayload]);

    if (error) {
        console.error('ERRO CAPTURADO:', error.message);
        if (error.details) console.error('DETALHES:', error.details);
    } else {
        console.log('✅ Inserido com sucesso? O erro sumiu para este payload?');
    }
}
